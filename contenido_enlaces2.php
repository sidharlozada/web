...
<span class="titulo" >Contenido de Enlaces</span><div class="linea" ></div>
   <div  class = "row" style="margin-top:20px">
      ...
      <form class="form-signin" role="form" method="POST" action="consultas.php" name="noticias" id="noticias">
        <table border="0"  align="" class="" width="100%">
            ...
            <tr>
              <td colspan="2">
                <table width="100%" border="0">
                  <td  width="30%">     
                  <? $q="SELECT * FROM public.tipo_publicaciones WHERE id=5 and estatus='0' ";
                     $r =  $conn->Execute($q);
                  ?>
                    <select name="tipo_publicacion" id="tipo_publicacion" class="selectpicker form-control"  style="font-size: 14px;">
                      <!-- <option value="0">Seleccione tipo de publicación</option> -->
                      <?while(!$r->EOF){?>
                      <option value="<?=$r->fields['id']?>" SELECTED><?=$r->fields['descripcion']?></option>
                        <?$r->movenext();
                      }?>
                    </select>            
                  </td>
                  <td width="5%"> &nbsp;</td>
                  <td width="30%"><input type="hidden" name="estatus" id="estatus" value="2">
                ...
                <table width="100%">
                  ...
                  <td>
                    <div  id="sandbox-container" >
                      <input type="text" class="form-control" placeholder="Fecha" name="fecha" id="fecha" value="<?=date("d/m/Y")?>">
                    </div>
                  </td>
               </table>
              </td>
            ...
            <tr>
              <td> <input type="hidden" name="url" id="url" value="">
        ...
        $.ajax({
                ...
                success:  function (response) {//Aqui se recibe la informacion si se guardo o no los registros
                 ...
                 if (response==1){
                    ...
                    $("#env").hide();
                    alertify.error("Error Registrando el Siniestro");
