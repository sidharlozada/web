...
<!-- Inicio Noticias Secundarias-->
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="row" >
           <?$noti = new noticias;
             $resp = $noti->get_busqueda2($conn,$start_from,$record_per_page,$find);
             $cont = 0;
             foreach($resp as $respuesta){
             $cont++;
             if ($respuesta->tipo_publicacion==1 || $respuesta->sub_noti!=0) {
               $img ="img_not/thumbs/";
             }else{
                $img ="img/";
             }
           ...
           ?>
            <div class="col-md-4 resumen_notsi" style="height: auto;"   >
                ...
                <a style="text-decoration: none;color: #423F3F" href="contenido_noticias.php?id=<?=$respuesta->id;?>">
                      <img src="<?=$img.$respuesta->url;?>" width="200px" height="80px">
                      <br><br>
...
function noticia_hist(pagina, buscar) {
  ...
  $('#contenido').load("contenido_enlaces.php?pagina="+pagina+"&find="+buscar, function() {
    $(this).fadeIn();
  });
  //window.location.replace('#topeNoticia');
}
