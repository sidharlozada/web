...
if(!empty($id_operacion) and !empty($id_usuario)){//echo " id_usuario=". $id_usuario." id_operacion=".$id_operacion;
...
?>
	<span class="titulo_maestro" ><b><?=$modulo->descripcion?></b></span>
	<ul>
<?
	$j = 1;
...
?>
		<li><input type="checkbox" id="permiso_<?=$i?>_<?=$j?>"  onclick="setPermisos(<?=$id_usuario?>, <?=$permiso->id?>);" <?=$checked?> /><span onclick="new Effect.Highlight(this.parentNode);" for="permiso_<?=$i?>_<?=$j?>"><?= !empty($permiso->nom_padre) ? $permiso->nom_padre." \\ ".$permiso->descripcion : $permiso->descripcion?> <?=$tipo?></span></li>
<? $j++; }
}else{
